 ...
 * @author Lxgxgxgxg
 * @version 1.0.0
 * @ClassName Kurtosis.java
 * @Description TODO
 * @createTime 2021年11月11日 21:55:00
 */
public interface Kurtosis {
     ...
     */
    static BigDecimal calculateKurtosis(HashMap<String, HashMap<String, BankAudit>> allTransactionMap, String str, BigInteger bankQuadraAssetsSum,
                                        BigInteger bankQuadraComRandomSum, BigInteger bankTripleAssetsSum, BigInteger bankDoubleAssetsSum,
                                        BigDecimal transactionAverage, BigInteger transactionTime, BigDecimal variance,
                                        BigInteger[] arr) {

        BigDecimal kurtosis = new BigDecimal(0.0);
         ...
         * 从账本获取四次资产的和
         */
        BigInteger qCHSum = quadraComHomomorphismSum(allTransactionMap, str, arr);
        System.out.println("qCHSum:"+qCHSum);
        BigInteger newQCHSum = arr[2].modPow(bankQuadraAssetsSum, arr[0]).multiply(arr[3].modPow(bankQuadraComRandomSum, arr[0])).mod(arr[0]);
        System.out.println("newQCHSum:"+newQCHSum);
        if (qCHSum.equals(newQCHSum)){
            /**
             * 四次资产和的值写进文件
             */
...
//        file5.close();
        ...
        BigDecimal bTAV = transactionAverage;
         ...
         */
        //分子
        BigDecimal molecular = bankQuadraAssetsSum1.divide(bTT, 8, BigDecimal.ROUND_HALF_UP).subtract((bankTripleAssetsSum1.multiply(bTAV).
                multiply(BigDecimal.valueOf(4))).divide(bTT, 8, BigDecimal.ROUND_HALF_UP)).add((bankDoubleAssetsSum1.multiply((bTAV.pow(2).
                multiply(BigDecimal.valueOf(6))))).divide(bTT, 8, BigDecimal.ROUND_HALF_UP)).subtract(bTAV.pow(4).multiply(BigDecimal.valueOf(3)));
        //分母
        BigDecimal denominator = variance.pow(2);
